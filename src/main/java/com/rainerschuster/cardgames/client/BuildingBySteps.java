...
public abstract class BuildingBySteps extends Building {
	...
	protected boolean accepts(final Card card1, final Card card2) {
		if (card1 == null) {
			logger.log(Level.INFO, "Card 1 must not be null!");
		}
		if (card2 == null) {
			logger.log(Level.INFO, "Card 2 must not be null!");
		}
		Objects.requireNonNull(card1);
		Objects.requireNonNull(card2);
		logger.log(Level.INFO, "Checking suit.");
		// Check suit order
		switch (suit) {
		case Suit.NULL:
			logger.log(Level.INFO, "Checking suit 'NULL'.");
			break;
		case Suit.SUIT:
			logger.log(Level.INFO, "Checking suit 'SUIT'.");
			if (!suitOrderSuit(card1, card2)) {
				return false;
			}
			break;
		case Suit.COLOR:
			logger.log(Level.INFO, "Checking suit 'COLOR'.");
			if (!suitOrderColor(card1, card2)) {
				return false;
			}
			break;
		case Suit.ALTERNATING:
			logger.log(Level.INFO, "Checking suit 'ALTERNATING'.");
			if (!suitOrderAlternating(card1, card2)) {
				return false;
			}
			break;
		case Suit.OTHERSUIT:
			logger.log(Level.INFO, "Checking suit 'OTHERSUIT'.");
			if (!suitOrderOtherSuit(card1, card2)) {
				return false;
			}
			break;
		default:
			logger.log(Level.INFO, "Invalid suit!");
			assert false;
			break;
		}

		logger.log(Level.INFO, "Checking direction.");
		// Check direction
		switch (direction) {
		case Direction.NULL:
			logger.log(Level.INFO, "Checking direction 'NULL'.");
			break;
		case Direction.UP:
			logger.log(Level.INFO, "Checking direction 'UP'.");
			if (card1.getCardValue() > card2.getCardValue()) {
				return false;
			}
			break;
		case Direction.DOWN:
			logger.log(Level.INFO, "Checking direction 'DOWN'.");
			if (card1.getCardValue() < card2.getCardValue()) {
				logger.log(Level.INFO, "Building in direction 'DOWN' denied.");
				return false;
			}
			break;
		default:
			logger.log(Level.INFO, "Invalid direction!");
			assert false;
			break;
		}

		logger.log(Level.INFO, "Checking step.");
